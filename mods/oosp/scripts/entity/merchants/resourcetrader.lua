...
local config = require ("mods/oosp/config/oosp")
...
function ResourceDepot.initialize()
    ...
    if onServer() then
        stock = getStartingResources()
        config.debugPrint(4, Entity().name.." CreationStock",stock)
        -- resource shortage
        shortageTimer = -random():getInt(15 * 60, 60 * 60)
...
function getStartingResources()
    math.randomseed(Sector().seed +Entity().index.number)
    local st = {}
    ...
    for i = 1, NumMaterials() do
        st[i] = math.max(0, probabilities[i - 1] - 0.1) * (getInt(5000, 10000) * Balancing_GetSectorRichnessFactor(x, y))
    end
